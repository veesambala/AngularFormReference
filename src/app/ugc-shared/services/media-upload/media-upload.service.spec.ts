...
describe('Service: MediaUploadService', () => {
  ...
  it('Create MediaUpload called when it was called already', () => {
    ...
    // expect(console.error).toThrow('media upload instance already created. Call resetMediaItems() instead.');

    expect(() => {
      _service.createMediaUpload();
    }).toThrow('media upload instance already created. Call resetMediaItems() instead.');
    // expect(_service.createMediaUpload).toHaveBeenCalledWith('media upload instance already created. Call resetMediaItems() instead.');
  });

  it('ResetMediaItems called without createMediaUpload', () => {
    expect(() => {
      _service.resetMediaItems();
    }).toThrow('reset called on an invalid media-upload instance');
  });
