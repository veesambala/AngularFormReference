...
import { UgcC11nJson, CustomizationFactory, UgcC11n, Locales } from '../../factories/customization.factory';

/**
 * @type {boolean} only applies if IS_LOCALHOST = true; true if it should load the bubble customization, false to load the master (gives invalid bubble)
 */
...
export class UgcCustomizationService {
  public static masterUrl(): string {
    return window.location.origin + '/ugc_c11n/master/c11n.json'
      + (StaticUtils.queryParams.preview ? '?iw=' + (Math.random()) : '');
  }
  ...
  public static bubbleUrl(): string {
    return window.location.origin + '/ugc_c11n/bubbles/'
      + StaticUtils.queryParams.bi + '/c11n.json'
      + (StaticUtils.queryParams.preview ? '?iw=' + (Math.random()) : '');
  }
  ...
  /**
   * @param _http Class used for making get requests to load the customization file from the server
   */
  constructor(
    private _http: Http
  ) {
  ...
  private loadFromServer(): Promise<any> {
    return new Promise((resolve, reject) => {
      let bubbleRequest = StaticUtils.queryParams.bi ? this.bubbleLoader() : Observable.of(null);
  ...
  private bubbleLoader(): Observable<any> {
    let url: string = StaticUtils.IS_LOCALHOST && LOAD_LOCALHOST_BUBBLE ?
      UgcCustomizationService.mockMasterUrl() : UgcCustomizationService.bubbleUrl();
